...
export async function action({ request, params }: ActionFunctionArgs) {
	...
	const { data: surveyAsign, error: surveyAsignError } = await supabase
		.from("surveys_asigns")
		.select("*")
		.eq("survey_id", params.survey)
		.single();
	...
	const { error: answerCreateError, data: createdAnswer } = await supabaseAdmin
	...
	const { error: updateAsignError, data: updatedAsign } = await supabaseAdmin
		.from("surveys_asigns")
		.update({
			answer_id: createdAnswer.id,
		})
		.eq("id", surveyAsign.id)
		.select();
	if (updateAsignError) {
